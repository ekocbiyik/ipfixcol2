...
File::File(const struct cfg_file &cfg, ipx_ctx_t *ctx) : Output(cfg.name, ctx) {
    ...
    if (cfg.window_size < _WINDOW_MIN_SIZE) {
        throw std::runtime_error("(File output) Window size is too small (min. size: " + std::to_string(_WINDOW_MIN_SIZE) + ")");
    }
    ...
    if (cfg.window_align) {
        // Window alignment
        _thread->window_time = (_thread->window_time / _thread->window_size) * _thread->window_size;
    }

    // Create directory & first file
    void *new_file = file_create(ctx, _thread->storage_path, _thread->file_prefix, _thread->window_time, _thread->m_calg);
    if (!new_file) {
        delete _thread;
 ...
 */
int File::process(const char *str, size_t len) {
 ...
 * \return On success returns 0. Otherwise returns non-zero value.
 */
int File::dir_create(ipx_ctx_t *ctx, const std::string &path) {
    ...
    while (!stop && (pos = tmp.find_last_of('/', pos)) != std::string::npos) {
        ...
        if (aux_str == "/") {
            // Root
            IPX_CTX_ERROR(ctx, "(File output) Failed to create a storage directory '%s'.", path.c_str());
            return 1;
        }
        ...
        switch (errno) {
            ...
            default:
                ...
                IPX_CTX_ERROR(ctx, "(File output) Failed to create a directory %s (%s).", aux_str.c_str(), err_str);
                return 1;
        }
    }
 ...
 */
void *File::file_create(ipx_ctx_t *ctx, const std::string &tmplt, const std::string &prefix, const time_t &tm, calg m_calg) {
    char file_fmt[20];
