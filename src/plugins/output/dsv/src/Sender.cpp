...
/**
 * \brief Send a DSV record
 * \param[in] str DSV Record to send
 * \param[in] len Size of the record
 * \return Always #IPX_OK
 */
int Sender::process(const char *str, size_t len) {
    if (sd == INVALID_FD) {
        ...
        connection_time = now;
        if (connect() != IPX_OK) {
            IPX_CTX_WARNING(_ctx, "(Send output) Reconnection to '%s:%"
                    PRIu16
                    "' failed! "
                    "Trying again in %d seconds.", params.addr.c_str(), params.port, int(RECONN_DELAY));
            return IPX_OK;
        } else {
            IPX_CTX_INFO(_ctx, "(Send output) Successfully connected to '%s:%"
                    PRIu16
...
int Sender::connect() {
    ...
    freeaddrinfo(result);
    if (ptr == nullptr) {
        IPX_CTX_ERROR(_ctx, "(Send output) Unable to connect to '%s:%"
                PRIu16
                "'!", params.addr.c_str(), params.port);
        sd = INVALID_FD;
        return IPX_ERR_DENIED;
 ...
 */
enum Sender::Send_status Sender::send(const char *str, size_t len) {
    ssize_t now;
    size_t todo = len;
    const char *ptr = str;
    ...
    while (todo > 0) {
        ...
        if (now == -1) {
            ...
            IPX_CTX_INFO(_ctx, "(Send output) Destination '%s:%"
                    PRIu16
                    "' disconnected: %s", params.addr.c_str(), params.port, err_str);
            return SEND_FAILED;
        }
