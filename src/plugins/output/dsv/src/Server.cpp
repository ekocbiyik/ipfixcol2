...
Server::Server(const struct cfg_server &cfg, ipx_ctx_t *ctx) : Output(cfg.name, ctx) {
    ...
    if ((ret_val = getaddrinfo(NULL, port.c_str(), &hints, &servinfo)) != 0) {
        throw std::runtime_error("(Server output) Server initialization failed (" + std::string(gai_strerror(ret_val)) + ")");
    }
...
void *Server::thread_accept(void *context) {
    ...
    while (!acc->stop) {
        ...
        IPX_CTX_INFO(acc->ctx, "(Server output) Client connected: %s", get_client_desc(client_addr).c_str());
 ...
 */
enum Server::Send_status Server::msg_send(const char *data, ssize_t len, client_t &client) {
    ssize_t now;
    ssize_t todo = len;
    const char *ptr = data;
    ...
    while (todo > 0) {
        ...
        if (now == -1) {
            ...
            char buffer[128];
            const char *err_str = strerror_r(errno, buffer, 128);
            IPX_CTX_INFO(_ctx, "(Server output) Client disconnected: %s (%s)", get_client_desc(client.info).c_str(), err_str);
            return SEND_FAILED;
        }
     ...
     * avoid invalid DSV format. Temporary string should be here, because it is
...
int Server::process(const char *str, size_t len) {
    ...
    if (_acceptor->new_clients_ready) {
        ...
        _clients.insert(_clients.end(), _acceptor->new_clients.begin(), _acceptor->new_clients.end());
