...
static const struct fds_xml_args args_print[] = {FDS_OPTS_ELEM(PRINT_NAME, "name", FDS_OPTS_T_STRING, 0), FDS_OPTS_END};

/** Definition of the \<server\> node  */
static const struct fds_xml_args args_server[] = {FDS_OPTS_ELEM(SERVER_NAME, "name", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(SERVER_PORT, "port", FDS_OPTS_T_UINT, 0), FDS_OPTS_ELEM(SERVER_BLOCK, "blocking", FDS_OPTS_T_BOOL, 0), FDS_OPTS_END};

/** Definition of the \<send\> node  */
static const struct fds_xml_args args_send[] = {FDS_OPTS_ELEM(SEND_NAME, "name", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(SEND_IP, "ip", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(SEND_PORT, "port", FDS_OPTS_T_UINT, 0), FDS_OPTS_ELEM(SEND_PROTO, "protocol", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(SEND_BLOCK, "blocking", FDS_OPTS_T_BOOL, 0), FDS_OPTS_END};

/** Definition of the \<file\> node  */
static const struct fds_xml_args args_file[] = {FDS_OPTS_ELEM(FILE_NAME, "name", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(FILE_PATH, "path", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(FILE_PREFIX, "prefix", FDS_OPTS_T_STRING, 0), FDS_OPTS_ELEM(FILE_WINDOW, "timeWindow", FDS_OPTS_T_UINT, 0), FDS_OPTS_ELEM(FILE_ALIGN, "timeAlignment", FDS_OPTS_T_BOOL, 0), FDS_OPTS_ELEM(FILE_COMPRESS, "compression", FDS_OPTS_T_STRING, FDS_OPTS_P_OPT), FDS_OPTS_END};

/** Definition of the \<outputs\> node  */
static const struct fds_xml_args args_outputs[] = {FDS_OPTS_NESTED(OUTPUT_PRINT, "print", args_print, FDS_OPTS_P_OPT | FDS_OPTS_P_MULTI), FDS_OPTS_NESTED(OUTPUT_SERVER, "server", args_server, FDS_OPTS_P_OPT | FDS_OPTS_P_MULTI), FDS_OPTS_NESTED(OUTPUT_SEND, "send", args_send, FDS_OPTS_P_OPT | FDS_OPTS_P_MULTI), FDS_OPTS_NESTED(OUTPUT_FILE, "file", args_file, FDS_OPTS_P_OPT | FDS_OPTS_P_MULTI), FDS_OPTS_END};

/** Definition of the \<params\> node  */
static const struct fds_xml_args args_params[] = {FDS_OPTS_ROOT("params"), FDS_OPTS_ELEM(FMT_TFLAGS, "tcpFlags", FDS_OPTS_T_STRING, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_TIMESTAMP, "timestamp", FDS_OPTS_T_STRING, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_PROTO, "protocol", FDS_OPTS_T_STRING, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_UNKNOWN, "ignoreUnknown", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_OPTIONS, "ignoreOptions", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_NONPRINT, "nonPrintableChar", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_NUMERIC, "numericNames", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_OCTETASUINT, "octetArrayAsUint", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_BFSPLIT, "splitBiflow", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_DETAILEDINFO, "detailedInfo", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_ELEM(FMT_TMPLTINFO, "templateInfo", FDS_OPTS_T_BOOL, FDS_OPTS_P_OPT), FDS_OPTS_NESTED(OUTPUT_LIST, "outputs", args_outputs, 0), FDS_OPTS_END};
 ...
 */
bool Config::check_ip(const std::string &ip_addr) {
    in_addr ipv4;
    in6_addr ipv6;

    return (inet_pton(AF_INET, ip_addr.c_str(), &ipv4) == 1 || inet_pton(AF_INET6, ip_addr.c_str(), &ipv6) == 1);
}
 ...
 */
bool Config::check_or(const std::string &elem, const char *value, const std::string &val_true, const std::string &val_false) {
    ...
    // Error
    throw std::invalid_argument("Unexpected parameter of the element <" + elem + "> (expected '" + val_true + "' or '" + val_false + "')");
}
 ...
 */
void Config::parse_print(fds_xml_ctx_t *print) {
 ...
 * \throw invalid_argument or runtime_error
 */
void Config::parse_server(fds_xml_ctx_t *server) {
 ...
 * \throw invalid_argument or runtime_error
 */
void Config::parse_send(fds_xml_ctx_t *send) {
    ...
    while (fds_xml_next(send, &content) != FDS_EOC) {
        switch (content->id) {
            ...
            case SEND_PROTO:
                assert(content->type == FDS_OPTS_T_STRING);
                output.proto = check_or("protocol", content->ptr_string, "UDP", "TCP") ? cfg_send::SEND_PROTO_UDP : cfg_send::SEND_PROTO_TCP;
                break;
            case SEND_BLOCK:
                assert(content->type == FDS_OPTS_T_BOOL);
                output.blocking = content->val_bool;
    ...
    if (output.addr.empty() || !check_ip(output.addr)) {
        throw std::runtime_error("Value of the element <ip> of the output <send> '" + output.name + "' is not a valid IPv4/IPv6 address");
    }
 ...
 * \throw invalid_argument or runtime_error
 */
void Config::parse_file(fds_xml_ctx_t *file) {
    // Set defaults
        ...
        switch (content->id) {
            ...
            case FILE_WINDOW:
                ...
                if (content->val_uint > UINT32_MAX) {
                    throw std::invalid_argument("Windows size must be between 0.." + std::to_string(UINT32_MAX) + "!");
                }
    ...
    if (output.path_pattern.empty()) {
        throw std::runtime_error("Element <path> of the output '" + output.name + "' must be defined!");
    }
 ...
 * \throw invalid_argument or runtime_error
 */
void Config::parse_outputs(fds_xml_ctx_t *outputs) {
    const struct fds_xml_cont *content;
