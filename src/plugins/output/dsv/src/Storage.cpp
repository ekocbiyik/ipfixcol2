...
Storage::Storage(const ipx_ctx_t *ctx, const struct cfg_format &fmt) : m_ctx(ctx), m_format(fmt) {
    // Prepare the buffer
 ...
 * \throws bad_alloc in case of a memory allocation error
 */
void Storage::buffer_append(const char *str) {
...
void Storage::output_add(Output *output) {
 ...
 */
const char *Storage::session_src_addr(const struct ipx_session *ipx_desc, char *src_addr, socklen_t size) {
...
/**
 * \brief Convert template record to DSV string
 *
 ...
 */
void Storage::convert_tmplt_rec(struct fds_tset_iter *tset_iter, uint16_t set_id, const struct fds_ipfix_msg_hdr *hdr) {
    ...
    // Iteration through the fields and converting them to DSV string
 ...
 */
int Storage::convert_tset(struct ipx_ipfix_set *set, const struct fds_ipfix_msg_hdr *hdr) {
...
int Storage::records_store(ipx_msg_ipfix_t *msg, const fds_iemgr_t *iemgr) {
 ...
 * @param[in] hdr   Message header of IPFIX record
 */
void Storage::addDetailedInfo(const struct fds_ipfix_msg_hdr *hdr) {
...
/**
 * \brief Convert an IPFIX record to DSV string
 *
 * For each field in the record, try to convert it into DSV format. The record is stored
 ...
 * \throw runtime_error if the DSV converter fails
 */
void Storage::convert(struct fds_drec &rec, const fds_iemgr_t *iemgr, fds_ipfix_msg_hdr *hdr, bool reverse) {
    // Convert the record
    ...
    if (rc < 0) {
        throw std::runtime_error("Conversion to DSV failed (probably a memory allocation error)!");
    }
    ...
    if (m_format.detailed_info) {
        ...
        // Add detailed info to DSV string
        addDetailedInfo(hdr);

        // Add template ID to DSV string
        char field[LOCAL_BSIZE];
